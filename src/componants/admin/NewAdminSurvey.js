...
export default function NewAdminSurvey() {
  ...
  const handleQuestionChange = (index, event) => {
    ...
    // If the question type changes, reset options if it's not checkbox/radio, or ensure at least one option exists if it is.
    if (name === "type") {
      if (value !== "checkbox" && value !== "radio") {
        question.options = [""]; // Reset options if the type is not checkbox or radio
  ...
  const addQuestion = () => {
    ...
    setQuestionError(false); // Reset the question count error when a new question is added
  };
  ...
  const deleteQuestion = (index) => {
    ...
    if (newQuestions.length === 0) {
      setQuestionError(true); // Set the question count error if all questions are deleted
    }
  };
  ...
  const handleSurveySubmit = () => {
    ...
    request("POST", "/createSurvey", surveyData)
        .then((response) => {
  ...
  const handleOptionChange = (questionIndex, optionIndex, event) => {
    ...
    newQuestions[questionIndex].optionError = false; // Reset option error on change
    setQuestions(newQuestions);
  };
  ...
  const addOption = (questionIndex) => {
    ...
    newQuestions[questionIndex].optionError = false; // Reset option error when a new option is added
    setQuestions(newQuestions);
  };
  ...
  return (
      <div className="container mt-4">
        ...
        <h5 className="mb-3">Create Survey</h5>
        <div className="mb-3">
          <input
              type="text"
                    ...
                    <div className="mt-3">
                      ...
                      {q.optionError && (
                          <div className="text-danger mt-2">
                            At least one option is required for this question.
                          </div>
                      )}
                    </div>
                )}
        ...
        {questionError && (
            <div className="text-danger mb-3">
              You must add at least one question to the survey.
            </div>
        )}
